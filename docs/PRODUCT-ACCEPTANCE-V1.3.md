# Paretto v1.3 product acceptance matrix
...
- **Deliberately outside v1.3** — useful scope that is not required for the
...
| --- | --- | --- |
| Learner sign-in, signup, recovery, logout, account management, anonymous-progress claim, and cross-browser synchronization | **Implemented + automated evidence** | Better Auth and D1 account routes live in `app/learner-auth.ts`, `app/api/account`, and `app/api/progress`. Account-scoped browser caches and anonymous claim are covered by `tests/learner-account-claim.test.ts`, `tests/browser-progress-cache-identity.test.ts`, `tests/account-lifecycle-ui.test.tsx`, and the seeded account Playwright journey. Email account creation/recovery correctly stays unavailable when transactional email is absent. |
...
| Controlled beta without transactional email/support delivery | **Implemented + automated evidence** | `LAUNCH_MODE=controlled-beta` may return HTTP 200 only when core web identity, database/schema, retention, and queues are healthy. It always reports `productionReady:false` with explicit email/support limitations. Config, secret, deployment, smoke, monitor, and health regressions cover the contract. |
| Strict public launch | **Implemented + automated evidence; provider evidence required** | `LAUNCH_MODE=public` requires valid sender/support variables, `RESEND_API_KEY`, password recovery, support delivery, healthy operations, and strict smoke. CLI smoke/monitor default to public, so they reject a controlled beta unless the narrower mode is explicit. Real message delivery still requires the verified domain/provider. |
| Production deployment and post-deploy smoke | **Implemented; live run and external repository controls required** | `.github/workflows/deploy.yml` permits production only from a `v*` tag, verifies the tag SHA is reachable from `main` and has a successful complete seven-job `CI` run before protected secrets, reruns browser/release gates, validates config/secrets, captures backup evidence before migration, deploys through the guard, and finishes with GET-only mode-aware smoke. Protected-tag policy and a passing live deployment tied to the exact commit remain external evidence gates. Independent environment approval with administrator bypass disabled remains required for a team-operated or public launch; an explicitly documented owner-operated controlled beta may accept the single-account governance risk without relaxing any automated gate or CMS author/approver separation. |
...
| Windows platform automation | **Implemented + automated evidence with one declared tool exception** | Thirteen HTTPS Chromium journeys run on a Windows Server hosted runner to catch path, shell, process, and Chromium issues. The seeded account journey remains mandatory in Linux Chromium, Firefox, and WebKit but is skipped on hosted Windows because Wrangler 4.114.0's local HTTPS runtime exits during first sign-in; its log is retained. This does not certify Windows account behavior, Windows 11, Edge, high contrast, Narrator, or physical hardware. |
...
## Deliberately outside the v1.3 controlled-beta claim
...
A public launch additionally requires strict `--mode public` smoke, verified
transactional/support delivery, distinct operational owners, current
