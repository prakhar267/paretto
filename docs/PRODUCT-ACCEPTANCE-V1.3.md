...
| Windows platform automation | **Implemented + automated evidence** | All current HTTPS Chromium journeys run on a Windows Server hosted runner against the direct Miniflare backend and disposable Node TLS boundary, including secure-origin and seeded account sign-in/claim/deletion coverage. Per-run lifecycle logs are retained. This does not certify Windows 11, Edge, high contrast, Narrator, or physical hardware. |
