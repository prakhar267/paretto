...
| --- | --- | --- |
| One consistent release identity | **Implemented + automated evidence** | Web, health endpoint, XcodeGen, generated Xcode project, and App Store metadata agree on version 1.5.1, native build 13, bundle identifier `com.paretto.app`, and planned tag `v1.5.1`. `npm run version:verify` fails on drift. |
