...
| Dependency advisories | **Resolved + fail-closed automated evidence** | Production and development audits report zero findings. Patched transitive `brace-expansion` releases revoked the former GHSA-mh99-v99m-4gvg exception; `scripts/verify-npm-audit.mjs` now rejects every advisory without an allowlist. |
