...
assert.equal(candidate.username, "", "Credentials are not allowed in the monitor URL.");
assert.equal(candidate.password, "", "Credentials are not allowed in the monitor URL.");
...
assert.equal(response.status, 200, "The health endpoint did not return HTTP 200.");
assert.match(
