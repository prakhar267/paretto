...
function request(init: RequestInit = {}) {
  return new Request("https://loquivo.test/api/progress", init);
}
...
describe("progress API", () => {
  ...
  beforeEach(async () => {
    ...
    const adminAuth = await createAdminTestAuth(["admin@loquivo.test"]);
    setCloudflareEnv({
      DB: database,
  ...
  it("reports missing runtime configuration and incomplete migrations", async () => {
    ...
    setCloudflareEnv({ DB: database, ADMIN_EMAILS: "admin@loquivo.test" });
