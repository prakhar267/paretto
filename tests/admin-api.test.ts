...
const ADMIN_EMAIL = "editor@loquivo.test";
const REVIEWER_EMAIL = "reviewer@loquivo.test";
let adminCookie = "";
let reviewerCookie = "";
const ADMIN_HEADERS = {
...
function learnerRequest(body: object, email = "learner@loquivo.test") {
  const token =
    email === "learner@loquivo.test" ? "L".repeat(43) : "S".repeat(43);
  return new Request("https://loquivo.test/api/support", {
    method: "POST",
...
describe("admin CMS and support APIs", () => {
  ...
  it("authorizes every admin operation and strictly validates draft creation", async () => {
    const anonymous = await CONTENT_LIST(
      new Request("https://loquivo.test/api/admin/content"),
    );
  ...
  it("supports revision-safe CRUD, publishing, audit, and public curriculum ETags", async () => {
    ...
    const curriculum = await CURRICULUM_GET(
      new Request("https://loquivo.test/api/curriculum"),
    );
    ...
    database.beforeContentSlugUpdate = () => {
      ...
      if (row) {
        database.content.set(row.id, {
          ...
          updated_by_email: "other-editor@loquivo.test",
        });
      }
    };
  ...
  it("creates privacy-safe learner support requests, rate limits abuse, and audits status changes", async () => {
    const anonymous = await SUPPORT_CREATE(
      new Request("https://loquivo.test/api/support", {
        method: "POST",
    ...
    expect(stored?.user_key).not.toContain("learner@loquivo.test");
