...
final class AppModel: ObservableObject {
    ...
    static let progressSuppressionKey = "loquivo.ignore-persisted-progress"
    static let authSuppressionKey = "loquivo.ignore-keychain-session"
    private static let legacyProgressSuppressionKey = "pas-a-pas.ignore-persisted-progress"
    private static let legacyAuthSuppressionKey = "pas-a-pas.ignore-keychain-session"
            ...
            do {
                ...
                self.repository = ProgressRepository(
                    fileURL: FileManager.default.temporaryDirectory
                        .appendingPathComponent("loquivo-unavailable-state.json")
                )
    ...
    private static func migrateLegacyDefaults(in defaults: UserDefaults) {
        let migrations = [
            (legacyProgressSuppressionKey, progressSuppressionKey),
            (legacyAuthSuppressionKey, authSuppressionKey),
        ]
    ...
    func load() async {
        ...
        if userDefaults.bool(forKey: Self.progressSuppressionKey) {
            ...
            if await clearPersistedProgress() {
                ...
                alertMessage = "An old local progress file could not be erased, so Loquivo will not load it."
            }
        } else {
            do {
