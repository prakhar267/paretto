        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                ZStack(alignment: .bottomLeading) {
                    RoundedRectangle(cornerRadius: 28).fill(Color.loquivoNavy)
                    VStack(alignment: .leading, spacing: 14) {
                        Text(region?.name ?? "Île-de-France")
                            .font(.caption.weight(.semibold))
                            .foregroundStyle(Color.loquivoGold)
                            .textCase(.uppercase)
                        ...
                        Button("Start lesson") { _ = model.startLearning() }
                            ...
                            .tint(.loquivoGold)
                            .foregroundStyle(Color.loquivoNavy)
                    }
                    .padding(24)
                }
                .frame(minHeight: 250)
    ...
    var body: some View {
        BrandCard {
            VStack(alignment: .leading, spacing: 10) {
                Image(systemName: symbol).font(.title2).foregroundStyle(Color.loquivoCoral)
