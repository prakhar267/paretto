...
3. Run `xcodegen generate --spec ios/Loquivo/project.yml`.
4. Open `ios/Loquivo/Loquivo.xcodeproj` and run the `Loquivo` scheme.
...
6. The complete SwiftUI source set and app integration tests can be built on
   macOS without the iOS SDK using `swift test --package-path ios/Loquivo`.
7. With Xcode installed, run the native unit and UI suites with:

   ```sh
   xcodebuild \
     -project ios/Loquivo/Loquivo.xcodeproj \
     -scheme Loquivo \
...
```sh
cp ios/Loquivo/Configuration/Local.xcconfig.example \
  ios/Loquivo/Configuration/Local.xcconfig
```
...
distribution, so Loquivo is a clean pre-release cutover. Compatibility probes
...
- Set `LOQUIVO_STAGING_API_BASE_URL` and `LOQUIVO_PRODUCTION_API_BASE_URL` to
...
- Enable Sign in with Apple for `com.loquivo.app`, select the correct
