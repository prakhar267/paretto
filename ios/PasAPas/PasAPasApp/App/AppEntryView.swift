...
struct AppEntryView: View {
    ...
    var body: some View {
        Group {
            if let startupError = model.startupError {
                ContentUnavailableView {
                    Label("Pas à Pas cannot start", systemImage: "exclamationmark.triangle")
                } description: {
                    Text(startupError)
                } actions: {
        ...
        .background(Color.pasBackground)
    ...
    var body: some View {
        GeometryReader { proxy in
            ScrollView {
                VStack(spacing: 28) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 34).fill(Color.pasNavy)
                        Text("p")
                            .font(.system(size: 96, weight: .black, design: .rounded))
                            .foregroundStyle(Color.pasCream)
                    }
