...
3. Run `xcodegen generate --spec ios/PasAPas/project.yml`.
4. Open `ios/PasAPas/PasAPas.xcodeproj` and run the `PasAPas` scheme.
...
6. The complete SwiftUI source set and app integration tests can be built on
   macOS without the iOS SDK using `swift test --package-path ios/PasAPas`.
7. With Xcode installed, run the native unit and UI suites with:

   ```sh
   xcodebuild \
     -project ios/PasAPas/PasAPas.xcodeproj \
     -scheme PasAPas \
...
```sh
cp ios/PasAPas/Configuration/Local.xcconfig.example \
  ios/PasAPas/Configuration/Local.xcconfig
```
...
- Set `PASAPAS_STAGING_API_BASE_URL` and `PASAPAS_PRODUCTION_API_BASE_URL` to
...
- Enable Sign in with Apple for `com.pasapas.french`, select the correct
